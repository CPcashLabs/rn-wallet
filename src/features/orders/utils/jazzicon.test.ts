...
const metamaskJazzicon = require("@metamask/jazzicon") as (diameter: number, seed: number) => FakeElement
...
describe("jazzicon", () => {
  ...
  it("adapts the official MetaMask jazzicon output for RN rendering", () => {
    ;(globalThis as typeof globalThis & { document?: unknown }).document = installFakeDocument()

    const diameter = 44
    const seed = 0x12345678
    const packageResult = metamaskJazzicon(diameter, seed)
  ...
  it("derives the same seed for hex and tron addresses used by the app", () => {
  ...
  it("falls back to deterministic seeds for empty and invalid inputs", () => {
