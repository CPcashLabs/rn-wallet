...
function isTronChainName(chainName: string) {
  return chainName.trim().toUpperCase().includes("TRON")
}

function resolveTransferChainType(chainName: string): "TRON" | "EVM" {
  return isTronChainName(chainName) ? "TRON" : "EVM"
}

function buildAddressRegexes(patterns: string[] | undefined, receiveChainName: string) {
...
function extractTransferAddress(input: string, regexes: RegExp[]) {
