...
import { exportNativeFile, pickNativeImage, readNativeFilePickerCapability, saveNativeImage } from "@/shared/native/nativeFilePickerModule"
...
export const fileAdapter: FileAdapter = {
  ...
  async exportFile(input) {
    ...
    try {
      const base64 = input.encoding === "base64" ? input.content : Buffer.from(input.content, "utf8").toString("base64")
      await exportNativeFile(input.filename, base64, input.mimeType)
