...
import { createSendTokenOrder } from "@/domains/wallet/transfer/services/transferApi"
...
import { getTransferOrderOptions, type TransferOrderOption } from "@/shared/exchange/services/exchangeApi"
import { parseDecimalInput } from "@/shared/exchange/utils/order"
...
type Props = NativeStackScreenProps<TransferStackParamList, "SendTokenScreen">

export function SendTokenScreen({ navigation }: Props) {
  ...
  return (
    <HomeScaffold canGoBack onBack={navigation.goBack} title={t("transfer.send.sendToken")} scroll={false}>
          ...
          <View style={styles.optionList}>
            {options.map(item => (
              ...
              >
                ...
                <Text style={[styles.optionBody, { color: theme.colors.mutedText }]}>{item.recvCoinSymbol || item.recvCoinCode}</Text>
              </Pressable>
            ))}
          </View>
        </SectionCard>
        ...
        <PrimaryButton
          ...
          onPress={() => {
            ...
            void (async () => {
              setSubmitting(true)
              try {
                const result = await createSendTokenOrder({
